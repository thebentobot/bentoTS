...
export const command: Command = {
	...
	category: `admin`,
	description:
		`Command to setup and manage role management on your server. Assign a specific channel where your users can assign roles by themselves. You can write custom instructions, add and delete roles as you like.\nSetup by following the order of the usage commands below. You need to first add the instructions message by using the message argument, then add roles, and then set the channel for role assignment.`,
	usage:
	...
	run: async (client, message, args): Promise<Message> => {
		...
		async function rolesChannelUpdate(message: Message) {
			...
			const newMessage = await roleChannelDiscord.send(MessageContent, embed)
