...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        if (args.length < 1) {
            return message.channel.send(`You must specify what you want to do with the caseglobal setting.\nUse \`${guildData.prefix}help caseglobal\` to see how to use this command.`);
        };

        const caseGlobalData = await caseGlobal.findOne({raw: true, where: {guildID: message.guild.id}})

        if (args[0] === 'server' && args[1] == 'status') {
            return message.channel.send(`Server info for global moderation cases are currently \`${caseGlobalData.serverName ? 'enabled' : 'disabled'}\` for this server`)
        }

        if (args[0] === 'reasons' && args[1] == 'status') {
            return message.channel.send(`Reasons for global moderation cases are currently \`${caseGlobalData.reason ? 'enabled' : 'disabled'}\` for this server`)
        }
