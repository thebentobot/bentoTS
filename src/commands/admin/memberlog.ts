...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        if (args.length < 1) {
            return message.channel.send(`You must specify what you want to do with the member log settings.\nUse \`${guildData.prefix}help memberlog\` to see how to use this command.`);
        };
        ...
        if (args[0] == 'channel') {
            ...
            if (memberLogData === null) {
                ...
                return message.channel.send(`Your member log channel was assigned! It is in <#${creatememberLogChannelData.channel}> `);
            } else {
        ...
        if (args[0] == 'delete') {
            try {
                ...
                return message.channel.send(`Your member log channel is deleted in Bento's database and Bento will from now on not log changes, updates, and matters about the users of the server.\nPlease use ${guildData.prefix}memberlog channel <channelID> to enable it again.`);
            } catch {
