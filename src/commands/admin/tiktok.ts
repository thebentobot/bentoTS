		...
		try {
			...
			if (args.length < 1) {
				return message.channel.send(
					`You must specify what you want to do with the TikTok setting.\nUse \`${guildData?.prefix}help tiktok\` to see how to use this command.`,
				)
			}

			if (args[0] === `status`) {
				return message.channel.send(
					`TikTok embedding is currently \`${guildData?.tiktok ? `enabled` : `disabled`}\` on this server`,
				)
			}
			...
			if (args[0] === `enable` || args[0] === `disable`) {
				...
				return message.channel.send(
					`\`${
						args[0]
					}\` is an invalid argument for this command.\nYou must specify if you want to **enable** or **disable** TikTok embedding for all messages on this server!\nTikTok content is currently \`${
						guildData?.tiktok ? `enabled` : `disabled`
					}\` on this server`,
				)
			}
		} catch (err) {
			console.log(`Error at tiktok.ts, server ${message.guild?.id}\n\n${err}`)
		}
	},
}
