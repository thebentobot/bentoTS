	...
	run: async (client, message, args): Promise<Message | undefined> => {
		...
		if (args[0] === `set`) {
			...
			try {
				...
				return message.channel.send(`Your role <@&${roleID?.id}> was set as an mute role.`, {
		...
		if (args[0] === `delete`) {
			try {
				...
				return message.channel.send(
					`Your mute role <@&${args[1]}> is now deleted in Bento's database and Bento will from now on not assign users with that role when they get muted by a mod.\nPlease use ${guildData?.prefix}muterole set <roleID> to set an mute role again.\nYou can't mute users without a mute role.`,
				)
			} catch {
