...
export const command: Command = {
	...
	run: async (client, message, args): Promise<Message> => {
		...
		async function userFunction(message: Message, user: User | GuildMember) {
			...
			const serverRankUser: object[] = []
			const globalRankUser: object[] = []
			const bentoRankUser: object[] = []
	...
	globalRankUser[0].level
			...
			const backgroundOpacity = Math.round(
				(userProfileData
					? userProfileData.BackgroundColourOpacity !== null
						? userProfileData.BackgroundColourOpacity / 100
						: 1
					: 1) * 255,
			...
			const descriptionOpacity = Math.round(
				(userProfileData
					? userProfileData.descriptionColourOpacity !== null
						? userProfileData.descriptionColourOpacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			...
			const overlayOpacity = Math.round(
				(userProfileData
					? userProfileData.overlayOpacity !== null
						? userProfileData.overlayOpacity / 100
						: 0.2
					: 0.2) * 255,
			).toString(16)
			...
			const sidebarOpacity = Math.round(
				(userProfileData
					? userProfileData.sidebarOpacity !== null
						? userProfileData.sidebarOpacity / 100
						: 0.3
					: 0.3) * 255,
			).toString(16)
			...
			const fmDivBGOpacity = Math.round(
				(userProfileData ? (userProfileData.fmDivBGOpacity !== null ? userProfileData.fmDivBGOpacity / 100 : 1) : 1) *
					255,
			).toString(16)
			...
			const fmSongTextOpacity = Math.round(
				(userProfileData
					? userProfileData.fmSongTextOpacity !== null
						? userProfileData.fmSongTextOpacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			...
			const fmArtistTextOpacity = Math.round(
				(userProfileData
					? userProfileData.fmArtistTextOpacity !== null
						? userProfileData.fmArtistTextOpacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			...
			const xpDivBGOpacity = Math.round(
				(userProfileData ? (userProfileData.xpDivBGOpacity !== null ? userProfileData.xpDivBGOpacity / 100 : 1) : 1) *
					255,
			).toString(16)
			...
			const xpTextOpacity = Math.round(
				(userProfileData ? (userProfileData.xpTextOpacity !== null ? userProfileData.xpTextOpacity / 100 : 1) : 1) *
					255,
			).toString(16)
			...
			const xpText2Opacity = Math.round(
				(userProfileData ? (userProfileData.xpText2Opacity !== null ? userProfileData.xpText2Opacity / 100 : 1) : 1) *
					255,
			).toString(16)
			...
			const xpBarOpacity = Math.round(
				(userProfileData ? (userProfileData.xpBarOpacity !== null ? userProfileData.xpBarOpacity / 100 : 1) : 1) * 255,
			).toString(16)
			...
			const xpBar2Opacity = Math.round(
				(userProfileData ? (userProfileData.xpBar2Opacity !== null ? userProfileData.xpBar2Opacity / 100 : 1) : 1) *
					255,
			).toString(16)
			...
			const xpDoneServerColour1Opacity = Math.round(
				(userProfileData
					? userProfileData.xpDoneServerColour1Opacity !== null
						? userProfileData.xpDoneServerColour1Opacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			const xpDoneServerColour2Opacity = Math.round(
				(userProfileData
					? userProfileData.xpDoneServerColour2Opacity !== null
						? userProfileData.xpDoneServerColour2Opacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			const xpDoneServerColour3Opacity = Math.round(
				(userProfileData
					? userProfileData.xpDoneServerColour3Opacity !== null
						? userProfileData.xpDoneServerColour3Opacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			const xpDoneGlobalColour1Opacity = Math.round(
				(userProfileData
					? userProfileData.xpDoneGlobalColour1Opacity !== null
						? userProfileData.xpDoneGlobalColour1Opacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			const xpDoneGlobalColour2Opacity = Math.round(
				(userProfileData
					? userProfileData.xpDoneGlobalColour2Opacity !== null
						? userProfileData.xpDoneGlobalColour2Opacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			const xpDoneGlobalColour3Opacity = Math.round(
				(userProfileData
					? userProfileData.xpDoneGlobalColour3Opacity !== null
						? userProfileData.xpDoneGlobalColour3Opacity / 100
						: 1
					: 1) * 255,
			).toString(16)
			...
			const userTimezone = userProfileData
				? userProfileData.timezone !== null
					? moment().tz(userProfileData.timezone).format(`ddd, h:mmA`)
					: ``
				: ``
			const userBirthday = userProfileData
				? userProfileData.birthday !== null
					? userTimezone
						? `, ${moment(new Date(userProfileData.birthday)).format(`MMM D`)} 🎂`
						: `${moment(new Date(userProfileData.birthday)).format(`MMM D`)} 🎂`
					: ``
				: ``
			...
			const bentoServerMemberData = await guildMemberDB.findAll({ raw: true, where: { guildID: `714496317522444352` } })
		...
		usernameEmbed ? usernameEmbed.recenttracks.track[0].artist[`#text`] : `lmao`,
            ...
            .xpDoneServer {
                ...
                height: 100%;
                width: ${
	(xpServer / ((((serverRankUser[0].level as number) * serverRankUser[0].level) as number) * 100)) * 100
}%;
            }
            ...
            .xpDoneGlobal {
                ...
                height: 100%;
                width: ${
	(xpGlobal / ((((globalRankUser[0].level as number) * globalRankUser[0].level) as number) * 100)) * 100
}%;
            }
