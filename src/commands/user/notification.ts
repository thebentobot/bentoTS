...
import { guild, initModels, notificationMessage, notificationMessageCreationAttributes } from '../../database/models/init-models';
...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        if (!args.length) {
            ...
            return message.channel.send(`You need to use an argument for this command.\nUse \`${guildData.prefix}help notification\` to get help with this command.`)
        }
        ...
        async function addNoti (message: Message, content: string): Promise<Message> {
            ...
            const createNoti = await notificationMessage.findOrCreate({raw: true, where: {userID: message.author.id, content: wordContent}, defaults: notiAttr})

            if (createNoti[1] === false) {
                return message.channel.send(`You have already created a notification with the content you listed.\nIf you want to enable it globally use \`${guildData.prefix}notification global <content>\` to enable/disable.`)
            } else {
                try {
        ...
        async function globalNoti (message: Message, content: string) {
            ...
            if (!listNoti) {
                return message.channel.send(`The notification you tried to enable/disable global notifications with doesn't exist.`)
            } else {
                await notificationMessage.update({global: listNoti.global === true ? false : true}, {where: {userID: message.author.id, content: content}})
                return message.channel.send(`${message.author} Your notification was just \`${listNoti.global === true ? `disabled` : `enabled`}\` globally.`)
            }
        }
    }
}