...
export const command: Command = {
	...
	run: async (client, message, args): Promise<Message | undefined> => {
		...
		async function addNoti(message: Message, content: string): Promise<Message | undefined> {
			try {
				...
				console.log(`Error at notification.ts' add function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function deleteNoti(message: Message, content: string) {
			try {
				...
				console.log(`Error at notification.ts' delete function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function listNoti(message: Message) {
			try {
				...
				console.log(`Error at notification.ts' list function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function globalNoti(message: Message, content: string) {
			try {
				...
				console.log(`Error at notification.ts' global function, server ${message.guild?.id}\n\n${err}`)
			}
		}
	},
}
