    ...
    run: async (client, message, args): Promise<any> => {
        ...
        async function weatherFunction (message: Message, input?: any) {
            ...
            const Embed = new MessageEmbed()
                .setColor('#EB6E4B')
                .setAuthor(userID ? (message.guild.members.cache.get(userID).nickname ? `${message.guild.members.cache.get(userID).nickname} (${message.guild.members.cache.get(userID).user.username + '#' + message.guild.members.cache.get(userID).user.discriminator})` : message.guild.members.cache.get(userID).user.username + '#' + message.guild.members.cache.get(userID).user.discriminator) : 'OpenWeather', userID ? message.guild.members.cache.get(userID).user.displayAvatarURL() : 'https://pbs.twimg.com/profile_images/1173919481082580992/f95OeyEW_400x400.jpg', userID ? null : 'https://openweathermap.org/')
