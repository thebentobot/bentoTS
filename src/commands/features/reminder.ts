...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        if (!args.length) {
            return message.channel.send(`If you need help with reminders, please use \`${guildData.prefix}help reminder\` to see instructions`)
        }
        ...
        async function remindTime (message: Message, amountOfTime: string, timeframe: string, reminder: string) {
            if (!amountOfTime) {
                return message.channel.send(`You haven't specified the amount of time.\nIf you need help with reminders, please use \`${guildData.prefix}help reminder\` to see instructions`)
            }

            if (!timeframe) {
                return message.channel.send(`You haven't specified the timeframe after ${amountOfTime}.\nIf you need help with reminders, please use \`${guildData.prefix}help reminder\` to see instructions`)
            }
            ...
            if (!reminder) {
                return message.channel.send(`You haven't specified what I should remind you of.\nIf you need help with reminders, please use \`${guildData.prefix}help reminder\` to see instructions`)
            }

            const remindDate = moment(new Date()).add(amountOfTime, timeframe as moment.unitOfTime.DurationConstructor).toDate()
            ...
            const reminderData = await remindDB.findOrCreate({raw: true, where: {userID: message.author.id, reminder: reminder}, defaults: reminderAttr}).catch(console.error)
            ...
            try {
                ...
                return await message.channel.send(`Reminder hasn't been set, because I can't send DM's to you.\nPlease check your privacy settings and try again.`)
            }
        }

        async function remindSchedule (message: Message, date: string, time: string, utc: string, reminder: string) {
            if (!date) {
                return message.channel.send(`You haven't specified the date for your reminder.\nIf you need help with reminders, please use \`${guildData.prefix}help reminder\` to see instructions`)
            }

            if (!time) {
                return message.channel.send(`You haven't specified the time for your reminder.\nIf you need help with reminders, please use \`${guildData.prefix}help reminder\` to see instructions`)
            }

            if (!utc) {
                return message.channel.send(`You haven't specified the timezone for your reminder.\nIf you need help with reminders, please use \`${guildData.prefix}help reminder\` to see instructions`)
            }
            ...
            if (moment(remindDateGathered, "DD-MM-YYYY HH:mm Z", true).isValid() === false) {
                return message.channel.send(`You need to specify a valid date to create a scheduled reminder. The format is DD-MM-YYYY HH:mm Z.\nE.g. \`25-11-2021 08:00 +02:00\``);
            }
        ...
        async function remindList (message: Message) {
            const reminderData = await remindDB.findAll({raw: true, where: {userID: message.author.id}, order: [['date', 'DESC']]});
