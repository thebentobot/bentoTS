...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        const data = await streamableAPI.get(`import?url=${url}&title=${title}`, {auth: {
            username: process.env.streamableUsername as string,
            password: process.env.streamablePassword as string
        }})

        if (data.status !== 200) return message.channel.send(`${message.author} Either your URL was invalid, or Streamable doesn't answer right now, try again 😔`)

        if (data.data?.status === 1) {
            let waitingMessage = await message.channel.send('Waiting for Streamable to process the video... ⌛') // https://stackoverflow.com/questions/53693209/get-message-id-of-the-message-sent-by-my-bot/53693480 

            let streamableStatus: boolean = false
            let TMRError: boolean = false
            let loopCount: number = 0 // adding seconds to each attempt, to avoid TMR
            ...
            while (streamableStatus === false) {
                ...
                if (loopCount === 5) {
                    waitingMessage.edit(`Approx. 10 minutes has gone by since Streamable started processing the video and I don't bother counting anymore 😒`)
                }
                ++loopCount
                const percentData = await streamableAPI.get(`videos/${data.data?.shortcode}`, {auth: {
                    username: process.env.streamableUsername as string,
                    password: process.env.streamablePassword as string
                }})
            ...
            if (TMRError === true || streamableStatus === false) {
                return message.channel.send(`${message.author} Error - Too many requests 😔 either your video is too big or Streamable is just stressed 🥺 I am sorry.`)
            }

            if (streamableStatus === true) {
                waitingMessage.delete()
                return message.channel.send(`${message.author} your streamable is done now! https://streamable.com/${data.data?.shortcode}`)
            }
        } else {
