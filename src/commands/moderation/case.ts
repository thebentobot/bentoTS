	...
	run: async (client, message, args): Promise<Message | void> => {
		...
		async function userCheck(
            ...
            WHERE ${caseType}."userID" = :user${!global ? ` AND ${caseType}."guildID" = :guild;` : `;`}`,
				...
				const queueEmbed = await message.channel.send(
					`Amount of ${caseType}s: ${currentPage + 1}/${(await embeds).length}`,
					(
						await embeds
					)[currentPage],
				)
						...
						if (message.guild?.id === `${current.guildID}`) {
							...
							if (message.guild?.id === `${current.guildID}`) {
								embed.addField(
									...
									`${
										actorNickname
											? `${actorNickname} (${actorUsername + `#` + actorDiscri})`
											: actorUsername + `#` + actorDiscri
									}`,
								)
				...
				async function generateOverviewEmbedding(input: {
					...
					embed.addField(
						...
						`This user has received ${
							input.bans.count > 1 || input.bans.count === 0 ? `${input.bans.count} bans` : `${input.bans.count} ban`
						}\n${
							!input.bans.rows.length
								? ``
								: `Last ban was on ${moment(input.bans.rows[0].date).format(`dddd, MMMM Do YYYY, HH:mm:ss A Z`)}`
						}`,
					)
					embed.addField(
						`Kicks`,
						`This user has received ${
							input.kicks.count > 1 || input.kicks.count === 0
								? `${input.kicks.count} kicks`
								: `${input.kicks.count} kick`
						}\n${
							!input.kicks.rows.length
								? ``
								: `Last kick was on ${moment(input.kicks.rows[0].date).format(`dddd, MMMM Do YYYY, HH:mm:ss A Z`)}`
						}`,
					)
					embed.addField(
						`Mutes`,
						`This user has received ${
							input.mutes.count > 1 || input.mutes.count === 0
								? `${input.mutes.count} mutes`
								: `${input.mutes.count} mute`
						}\n${
							!input.mutes.rows.length
								? ``
								: `Last mute was on ${moment(input.mutes.rows[0].date).format(`dddd, MMMM Do YYYY, HH:mm:ss A Z`)}`
						}`,
					)
					embed.addField(
						`Warnings`,
						`This user has received ${
							input.warnings.count > 1 || input.warnings.count === 0
								? `${input.warnings.count} warnings`
								: `${input.warnings.count} warning`
						}\n${
							!input.warnings.rows.length
								? ``
								: `Last warning was on ${moment(input.warnings.rows[0].date).format(
				...
				console.log(`Error at case.ts' usercheck function, server ${message.guild?.id}\n\n${err}`)
			}
		}

		async function caseCheck(message: Message, caseType: string, caseNumber: string): Promise<Message | undefined> {
			try {
				if (!caseType) {
				...
				if (isNaN(caseNumbered)) return message.channel.send(`Your case number is not a number`)
            ...
            WHERE ${caseType}."${caseType}Case" = :caseNumber;`,
				...
				if (caseType === `mute`) {
					...
					embed.addField(
						`Mute activity`,
						caseQuery[0].muteStatus === true ? `Currently muted for this case` : `Unmuted`,
					)
				}
				if (caseType === `kick`) {
				...
				console.log(`Error at case.ts' casecheck function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function caseEdit(
            ...
            WHERE ${caseType}."${caseType}Case" = :caseNumber;`,
				...
				console.log(`Error at case.ts' caseedit function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function caseDelete(message: Message, caseType: string, caseNumber: string): Promise<Message | void> {
            ...
            WHERE ${caseType}."${caseType}Case" = :caseNumber;`,
				...
				embed.setTitle(
					`Are you sure that you want to delete ${capitalize(caseType)} Case Number ${caseNumbered}: ${
						caseUserNickname
							? `${caseUserNickname}(${caseUsername}#${caseUserDiscri})`
							: `${caseUsername}#${caseUserDiscri}`
					}`,
				...
				if (caseType === `mute`) {
					...
					embed.addField(
						`Mute activity`,
						testCaseQuery[0].muteStatus === true ? `Currently muted for this case` : `Unmuted`,
					)
				}
				if (caseType === `kick`) {
				...
				if (message.guild.id === `${testCaseQuery[0].guildID}`) {
					...
					embed.addField(
						...
						testCaseQuery[0].note ? testCaseQuery[0].note : `No notes made for this case`,
					)
				}
				...
				console.log(`Error at case.ts' casedelete function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function caseSearch(
            ...
            ORDER BY CAST(${column} AS varchar) ILIKE :query, date DESC;`,
				...
				if (!testCaseQuery.length) {
					return message.channel.send(
						`No results found for the query ${query} under ${column} for the ${caseType} cases.`,
					)
				}
				...
				const queueEmbed = await message.channel.send(
					`Result: ${currentPage + 1}/${embeds.length}`,
					embeds[currentPage],
				)
				await queueEmbed.react(`⬅️`)
				await queueEmbed.react(`➡️`)
				...
				console.log(`Error at case.ts' casesearch function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function caseList(
			...
			try {
				...
				if (firstDate && secondDate) {
					if (moment(firstDate, `YYYY-MM-DD`, true).isValid() === false) {
						return message.channel.send(
							`You need to specify a valid start date to create a list. The format is YYYY-MM-DD`,
						)
					}
            ...
            ORDER BY date DESC;`,
				...
				if (!testCaseQuery.length) {
					return message.channel.send(
						`No ${caseType} case results found${firstDate ? ` between ${firstDate} and ${secondDate}` : `.`}`,
					)
				}
				...
				console.log(`Error at case.ts' caselist function, server ${message.guild?.id}\n\n${err}`)
			}
		}
	},
}
