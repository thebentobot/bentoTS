	...
	run: async (client, message, args): Promise<Message | GuildMember | undefined> => {
		...
		async function timedMute(message: Message, amountOfTime: string, timeframe: string, user: string, reason?: string) {
			try {
				...
				if (
					(message.guild?.members?.resolve(client?.user as ClientUser)?.roles?.highest?.position as number) <
					rolePosition
				) {
					return message.channel.send(
				...
				console.log(`Error at mute.ts' timed function, server ${message.guild?.id}\n\n${err}`)
			}
		}
		...
		async function regularMute(
			...
			try {
				...
				console.log(`Error at mute.ts' regular function, server ${message.guild?.id}\n\n${err}`)
			}
		}
	},
}
