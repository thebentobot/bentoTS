...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        try {
            unbannedUser = await client.users.fetch(`${args[0]}`).catch(() => console.error('fetch user error in unban.ts line 28')) as User
        ...
        if (!banned) {
            return message.channel.send(`**${(await client.users.fetch(unbannedUserID)).username}#${(await client.users.fetch(unbannedUserID)).discriminator}** isn't banned on this server.`);
        } else {
            try {
