...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        try {
            ...
            message.channel.send(`${message.guild.members.cache.get(`${warningUserID}`).nickname ? `${message.guild.members.cache.get(`${warningUserID}`).nickname} (${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator})` : `${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator}`} was successfully warned.\nCase number: ${warned.warningCase}.\nReason: ${reason}\nYou can add notes for this warning by using the case command, together with the case number.`)
        } catch {
            (await client.users.fetch(warningUserID)).send(`⚠ You have received a \`warning\` from **${message.guild.name}** ⚠\n**Reason**: ${reason}.\nThis is warning number ${warningCount.count} that you have received from this server.`)
            message.channel.send(`${message.guild.members.cache.get(`${warningUserID}`).nickname ? `${message.guild.members.cache.get(`${warningUserID}`).nickname} (${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator})` : `${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator}`} was successfully warned.\nCase number: ${warned.warningCase}.\nReason: ${reason}\nYou can add notes for this warning by using the case command, together with the case number.`)
        }
