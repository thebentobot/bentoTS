...
export const command: Command = {
    ...
    run: async (client, message, args): Promise<Message> => {
        ...
        try {
            warningUser = message.mentions.members.has(client.user.id) ? (message.mentions.members.size > 1 ? message.mentions.members.last() : message.member) : message.mentions.members.first() || await message.guild.members.fetch(args[0]);
        ...
        try {
            ...
            .setTitle(`${warningUser.nickname ? `${warningUser.nickname} (${warningUser.user.username}#${warningUser.user.discriminator})` : `${warningUser.user.username}#${warningUser.user.discriminator}`} was warned!`)
            .setDescription(`**Warning number ${warningCount.count}**\n**Reason**\n${reason ? reason : 'Reason not listed'}`)
            .addField('Username', warningUser.user.username + '#' + warningUser.user.discriminator)
            .addField('User ID', warningUser.id)
            .addField('Warned by', message.guild.members.cache.get(message.author.id).nickname ? `${message.guild.members.cache.get(message.author.id).nickname} (${message.guild.members.cache.get(message.author.id).user.username}#${message.guild.members.cache.get(message.author.id).user.discriminator})` : `${message.guild.members.cache.get(message.author.id).user.username}#${message.guild.members.cache.get(message.author.id).user.discriminator}`)
            .setFooter(`Warning Case Number: ${warned.warningCase}`)
            .setTimestamp();
            await logChannel.send(embed);
            try {
                (await client.users.fetch(warningUserID)).send(`⚠ You have received a \`warning\` from **${message.guild.name}** ⚠\n**Reason**: ${reason}.\nThis is warning number ${warningCount.count} that you have received from this server.`).catch(error => { console.error(`Could not send warning DM`, error)})
                return await message.channel.send(`**${message.guild.members.cache.get(`${warningUserID}`).nickname ? `${message.guild.members.cache.get(`${warningUserID}`).nickname} (${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator})` : `${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator}`}** was successfully **warned**.\n**Case number: ${warned.warningCase}**.\n**Reason:** ${reason}\nYou can add notes for this warning by using the case command, together with the case number.`)
            } catch {
                return await message.channel.send(`**${message.guild.members.cache.get(`${warningUserID}`).nickname ? `${message.guild.members.cache.get(`${warningUserID}`).nickname} (${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator})` : `${message.guild.members.cache.get(`${warningUserID}`).user.username + '#' + message.guild.members.cache.get(`${warningUserID}`).user.discriminator}`}** was successfully **warned**.\n**Case number: ${warned.warningCase}**.\n**Reason:** ${reason}\nYou can add notes for this warning by using the case command, together with the case number.`)
            }
        } catch {
