...
/*
Reason for avoidance of non-null assertion, and using optional chain operator instead:
ESLint warning:
"Consider using the optional chain operator `?.` instead. This opeator includes runtime checks, so it is safer than the compile-only non-null assertion operator. (@typescript-eslint/no-non-null-assertion)"
*/

export const event: Event = {
	name: `roleDelete`,
	run: async (client, role: Role): Promise<Message | void> => {
		initModels(database)
