...
import { initModels, modLog, autoRole, muteRole, guild, role as roleDB } from '../database/models/init-models';
    ...
    run: async (client, role: Role): Promise<any> => {
        ...
        if (roleData) {
            ...
            try {
                const log = await modLog.findOne({where: { guildID: role.guild.id}});
                const modLogChannel: TextChannel = client.channels.cache.get(`${log.channel}`) as TextChannel;
                await modLogChannel.send(`A deleted role called **${role.name}** was a role users could assign in the role management channel, and it has been deleted from Bento's database.\nRemember to update the role channel message by using \`${guildData.prefix}role update\`.`)
            } catch {
                return
            }
        }
