...
export const event: Event = {
    ...
    run: async (client, member) => {
        ...
        await guildMember.destroy({where: {userID: member.id}});
        ...
        try {
            const byeData = await bye.findOne({where: { guildID: member.id }});

            const channel = member.guild.channels.cache.get(byeData.channel)
